...
class CardViewHeroAdapter(private val listHero: ArrayList<Hero>) : RecyclerView.Adapter<CardViewHeroAdapter.CardViewViewHolder>() {
    ...
    override fun onBindViewHolder(holder: CardViewViewHolder, position: Int) {
        ...
        holder.btnShare.setOnClickListener {
            val name = listHero[position].name
            val detail = listHero[position].detail

            val intent = Intent()
            intent.action = Intent.ACTION_SEND
            intent.putExtra(Intent.EXTRA_TEXT, "eh tau gak sih tentang pahlawan yang namanya $name, nah ini dia diskripsi tentang dia: $detail ")
